...
VBCC_VASM_FLAGS := -m$(CPU) -quiet -Fhunk -nowarn=62 -I $(LIB_DIR) $(COMMON_DEFINES)
...
$(BUILD_DIR)/jetpac.exe: jetpac.has $(ASSET_OBJECTS) $(JETPAC_OBJECTS) $(STAR_OBJECT) | $(BUILD_DIR) check-tools
	...
	$(VASM) $(VASM_FLAGS) -D DISABLE_640x256=1 -D DISABLE_HAM=1 -D HEAP_MEMORY=131072 "$(BUILD_DIR)/jetpac.s" -o "$(BUILD_DIR)/jetpac.o"
